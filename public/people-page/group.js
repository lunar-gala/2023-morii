...
function make_group(gparent, gname, grole, gimg, gisperson) {
  ...
  if (grole == 'PRODUCERS') {
    ...
    if (gname.toUpperCase() == 'NATHAN WAI') {
      grole += ', MODEL';
    }
  ...
  let g = {
    ...
    y_origin: gy_origin,
    x_actual: gx_origin,
    y_actual: gy_origin, // initially at origin
    text_x: random(0, imgW),
    text_y: random(0, imgW),
...
function move_group(rpx, rpy) {
  ...
  if (rpx == null && this.wait > 20) {
    //groups are in normal positions
    this.wait++;
    ...
    if (dp != 0) {
      ...
      this.x_actual = abs_constrain(
        this.x_actual,
        this.x_target,
        this.x_target + (rpc / 250) * dirx * f
      );
      this.y_actual = abs_constrain(
        this.y_actual,
        this.y_target,
        this.y_target + (rpc / 250) * diry * f
      );
    }
  } else if (rpx != null) {
