...
// line object //
function create_line(limg, lnum, llink) {
  var l = {
    x: 0, y: 0,
    img: limg,
    num: lnum,
...
function preload() {
  ...
  let line_names = [ "arriba", "atrophy", "delicacy", "doodles", "kalopsia", 
                      "lapinata", "limbic", "noxmemoria", "paperdollls", 
                      "rewind", "secoulth", "weilai", "xiaoshi"];

  for (var i = 0; i < line_names.length; i++) {
    print(line_names[i]);
    let curr_img = loadImage('assets/' + line_names[i] + '.png');
    let curr_link = "./" + line_names[i] + '.html'; // probably needs changing
    LINES.push(create_line(curr_img, i, curr_link));
  }
